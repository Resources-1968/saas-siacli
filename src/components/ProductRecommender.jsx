...
const ProductRecommender = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            setIsLoading(true);
            try {
                const [productsData, nicheAnalysis] = await Promise.all([
    ...
    useEffect(() => {
        ...
        // Debounce slightly to avoid double-firing on initial load if needed, 
        // but for now direct call is fine as it's user-initiated change usually.
        // We skip the first run if we want, but initial load handles it.
        // Actually, initial load does getAllProducts. We should probably just use this effect for everything.

        if (!isLoading) { // Only run if initial load is done
            runAnalysis();
        }
    }, [selectedRegion, selectedSector, isLoading]);

    const handleAnalyze = () => {
        // Manual refresh if needed, but effect handles changes
        // We can force a re-run by just calling the service, but the effect is better.
        // For now, this button is redundant but we can keep it as "Forzar Análisis".
    };
